...
class PFLSwiftAlertView: UIView, Animationable {
    ...
    var confirmButtonTitle: String? {
        didSet {
            if let btn = confirmBtn {
                btn.setTitle(confirmButtonTitle, for: UIControlState())
            }
        }
    }
    ...
    required init(title: String? = "提示", message: String?, delegate: AnyObject?, cancelButtonTitle: String?, otherButtonTitle: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(PFLSwiftAlertView.keyboardWillShow(_:)), name: NSNotification.Name.UIKeyboardWillShow, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(PFLSwiftAlertView.keyboardDidHide(_:)), name:NSNotification.Name.UIKeyboardDidHide, object: nil)
    ...
    @objc fileprivate func keyboardWillShow(_ notification: Notification) {
        ...
        let kbHeight:CGFloat = ((notification as NSNotification).userInfo![UIKeyboardFrameBeginUserInfoKey] as! NSValue).cgRectValue.size.height
    ...
    var confirmBtnColor: UIColor = UIColor.red {
        didSet {
            ...
            confirmBtn.setTitleColor(confirmBtnColor, for: UIControlState())
        }
    }
    ...
    lazy fileprivate var confirmBtn: UIButton? = {
        if let confirmTitle = self.confirmButtonTitle {
            ...
            btn.tag = 2
            btn.titleLabel?.font = UIFont.systemFont(ofSize: fontSize + 1)
            btn.setTitle(confirmTitle, for: UIControlState())
            btn.setTitleColor(UIColor.red, for: UIControlState())
            return btn
        }
        else {
    ...
    fileprivate func getLabelHeight(_ label: UILabel) -> CGFloat {
        ...
        if (UIDevice.current.systemVersion as NSString).floatValue >= 7.0 {
            height = text.boundingRect(with: CGSize(width: label.frame.width, height: 1000), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSFontAttributeName:label.font], context: nil).size.height
        }
        else {
            height = text.size(attributes: [NSFontAttributeName:label.font]).height
        }
        return height
    }
    ...
    func show() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(dismissAlertView), name: NSNotification.Name.UIApplicationWillResignActive, object: nil)
    }
